...
export async function updateRule(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Use Prisma.AutomationRuleUpdateInput compatible type
    const updateData: any = {};
    if (req.body.name !== undefined) updateData.name = req.body.name;
    if (req.body.isActive !== undefined) updateData.isActive = req.body.isActive;
    if (req.body.actionConfig !== undefined) updateData.actionConfig = req.body.actionConfig;
    if (req.body.delayMinutes !== undefined) updateData.delayMinutes = req.body.delayMinutes;

    const updated = await prisma.automationRule.update({ where: { id: rule.id }, data: updateData });
  ...
  try {
    ...
    const [logs, total] = await Promise.all([
      prisma.automationLog.findMany({
        where,
        include: { automationRule: { select: { name: true, triggerEvent: true } } },
